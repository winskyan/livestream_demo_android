package io.agora.fastlive.fragment;
...
import io.agora.fastlive.presenter.IFastAudienceView;
...
public class FastLiveAudienceFragment extends FastLiveBaseFragment implements IFastAudienceView {
    ...
    public void onRtcRemoteVideoStateChanged(int uid, int state, int reason, int elapsed) {
        Log.i(RtcEventHandler.TAG, "onRtcRemoteVideoStateChanged uid: "+uid + " state: "+state);
    ...
    public void onRtcUserJoined(int uid, int elapsed) {
        Log.i(RtcEventHandler.TAG, "onUserJoined->" + uid);
    }
    ...
    public void onGetTokenSuccess(String token, int uid, boolean isRenew) {
        this.uid = uid;
        Log.i(RtcEventHandler.TAG, "uid: " + uid);
        rtcToken = token;
        if(isRenew) {
            renewToken(token);
        }else {
